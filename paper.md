...
keyword:
  ...
  - blinding
abstract: |
  Low-intensity transcranial direct current stimulation is a frequenty used non-invasiv brain stimulation method, which can modulate cortical excitability level in humans.
linenumbers: true
...
TDCS applied at 1 mA for 20 min is a frequently used stimulation protocol in the tDCS literature. It is assumed that effective blinding is maintained when FSF is used as active sham protocol. However, this assumption is based on 24 healthy volunteers and 23 chronic stroke patients with a mean age between 46.3 and 62.3 years [@gandiga2006transcranial]. Recent evidence indicates that the tDCS-induced cutaneous discomfort may depend on the age of the participants, and it is lower in older than in younger participants [@wallace2016perceived]. This difference in the sensitivity may lead to the possibility that younger volunteers could better distinguish between real and active sham stimulation protocols than older participants, which could compromise effective blinding in younger adults. Our analysis, based on subjective ratings from 192 healthy, young adults suggest that blinding is compromised for tDCS at 1 mA for 20 min. One possible explanation for this difference between the present and Gandiga's study is that our sample size was larger and the sample itself consisted of young, healthy adults. We found our results still suprising, since the present study used a 30s fade-in/out period, whereas Gandiga and colleagues' protocol used 10s fade-in and no fade-out period [@gandiga2006transcranial]. Note that longer fade-in/out periods are supposed to further improve the blinding efficacy of the FSF protocol. Taking together, these findings imply that developing new active sham protocol, which can effectively blind tDCS at 1 mA for 20 min is strongly recommended. 
