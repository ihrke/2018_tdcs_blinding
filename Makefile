...
${PAPER}.tex: ${PAPER}.md ${BIBLIOGRAPHY} ${TEMPLATE}
	${PANDOC} -s -f markdown -t latex  --filter /usr/local/bin/pandoc-fignos --filter filters/latex_textred.py --filter pandoc-citeproc ${USE_NATBIB} ${PAPER}.md -o ${PAPER}.tex --template ${TEMPLATE} --bibliography ${BIBLIOGRAPHY}

${SUPPLEMENT}.tex: ${SUPPLEMENT}.md ${BIBLIOGRAPHY} ${TEMPLATE}
	${PANDOC} -s -f markdown -t latex  --filter filters/latex_textred.py --filter /usr/local/bin/pandoc-fignos --filter pandoc-citeproc ${USE_NATBIB} ${SUPPLEMENT}.md -o ${SUPPLEMENT}.tex --template ${TEMPLATE} --bibliography ${BIBLIOGRAPHY}
